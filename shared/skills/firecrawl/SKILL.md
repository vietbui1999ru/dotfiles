...
name: firecrawl
description: |
  General web research and page extraction via Firecrawl. Use for URLs, articles,
  news, current events, broad web/image search, site mapping, crawling, and
  JavaScript-rendered pages. Do not use for official package/API documentation
  when Context7 is available, or for cross-repository implementation examples
  handled by `ketch code`. Replaces built-in WebFetch and WebSearch only within
  its general-web scope. See SKILL.md for routing and usage.
---

# Firecrawl CLI

Use the `firecrawl` CLI for general web search and extraction instead of built-in
WebFetch or WebSearch.

## Routing boundary

- **Context7 first**: official library/framework/SDK/CLI documentation, API syntax,
  configuration, migrations, and version-specific behavior.
- **`ketch code` first**: real implementation examples from public repositories.
- **Firecrawl first**: direct URLs, articles, news, current events, broad research,
  comparisons, site discovery, crawling, and JavaScript-rendered pages.

Use Firecrawl as a fallback when Context7 lacks the library/topic or Ketch finds no
useful code. Do not call multiple research tools for the same question unless the
primary tool fails or the user asks for corroboration.
