...
export default function piSession(pi: ExtensionAPI) {
	...
	pi.registerCommand("resume", {
		description:
			"Resume a session: /resume <file> or /resume to see active sessions",
		handler: async (args, ctx) => {
			...
			handler: async (args, ctx) => {
				...
				if (result.code === 0) {
					...
					if (cfg.obsidianBridge) {
						const fileMatch = result.stdout.match(/sessions\/(\S+)/);
						if (fileMatch) {
							ctx.ui.notify(
								"Use /open " + fileMatch[1] + " to view in Obsidian",
								"info",
							);
						}
					}
	...
	pi.registerCommand("review", {
		...
		handler: async (args, ctx) => {
			...
			ctx.ui.notify(
				...
				existsSync(reviewPath) ? "info" : "warning",
			);
		},
	});
