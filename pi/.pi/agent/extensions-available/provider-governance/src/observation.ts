...
function unknownPolicy(): ProviderObservationPolicy {
	return {
		...
		billing: "unknown",
		warnings: [
			"Provider class, authorization, and billing are unknown; no inference was made.",
		],
	};
}
...
export function inventoryModels(
	...
	return [...byProvider.entries()].map(([providerId, modelIds]) => {
		...
		const healthObservation = health[providerId] ?? {
		...
		return {
			...
			...policy,
			retryLabel:
				policy.providerClass === "acp-delegate"
...
export function formatProviderStatus(
	...
	lines.push(
		"No credentials, account identity, tool inputs, headers, or response bodies are displayed.",
	);
	return lines.join("\n");
}

export function anthropicExtraUsageWarning(
	providerId: string,
): string | undefined {
