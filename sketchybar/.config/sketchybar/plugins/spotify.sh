...
next () { osascript -e 'tell application "Spotify" to play next track'; update; }
back () { osascript -e 'tell application "Spotify" to play previous track'; update; }
play () { osascript -e 'tell application "Spotify" to playpause'; update; }
...
truncate_text() {
  ...
  if [ ${#text} -le "$max_length" ]; then echo "$text"
  else echo "${text:0:max_length}" | sed -E 's/\s+[[:alnum:]]*$//' | awk '{$1=$1};1' | sed 's/$/.../'
  fi
}
...
update() {
  ...
  if [ "$SPOTIFY_DISPLAY_CONTROLS" = "true" ]; then
    if [ "$state" = "playing" ]; then play_icon="􀊆"; else play_icon="􀊄"; fi
  fi
  ...
  if [ "$SPOTIFY_DISPLAY_CONTROLS" = "true" ]; then
    sketchybar --set spotify.shuffle icon.highlight=$shuffle \
...
routine() { case "$NAME" in "spotify.state") scroll ;; *) update ;; esac; }
